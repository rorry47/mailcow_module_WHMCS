...
class MailcowAPI
{
    ...
    // -----------------------------------------------------------------------
    // Domain stats (for Statistics tab)
    // -----------------------------------------------------------------------

    public function getDomainStats(string $domain): array
    {
        $result = $this->_request('GET', '/api/v1/get/domain/' . rawurlencode($domain));
        return is_array($result['body']) ? $result['body'] : [];
    }

    public function getMailboxes(string $domain): array
    {
        $result = $this->_request('GET', '/api/v1/get/mailbox/all/' . rawurlencode($domain));
        return is_array($result['body']) ? $result['body'] : [];
    }


    // -----------------------------------------------------------------------
    // Upgrade domain limits (used by upgrade form + InvoicePaidPre hook)
    // -----------------------------------------------------------------------

    public function upgradeDomain(string $domain, array $overrides): array
    {
        $attr = [
            'description'          => $domain,
            'aliases'              => isset($overrides['aliases'])    ? (int)$overrides['aliases']    : $this->aliases,
            'mailboxes'            => isset($overrides['mailboxes'])  ? (int)$overrides['mailboxes']  : $this->NUM_MAILBOXES,
            'defquota'             => isset($overrides['defquota'])   ? (int)$overrides['defquota']   : $this->DEFQUOTA,
            'maxquota'             => isset($overrides['maxquota'])   ? (int)$overrides['maxquota']   : $this->MAILBOXQUOTA,
            'quota'                => isset($overrides['quota'])      ? (int)$overrides['quota']      : $this->TOTAL_QUOTA,
            'backupmx'             => 0,
            'relay_all_recipients' => 0,
            'rl_value'             => $this->rl_value,
            'rl_frame'             => $this->rl_frame,
    ...
    // -----------------------------------------------------------------------
    // DKIM — regenerate (delete existing + create new)
    // -----------------------------------------------------------------------

    public function regenerateDkim(string $domain): array
    {
        // Delete existing DKIM key first, then generate new one
        $this->_request('POST', '/api/v1/delete/dkim', ['domains' => [$domain]]);
