...
function mailcow_ClientArea(array $params): string
{
    ...
    if ($tab === 'overview') {
        ...
        $dnsRows = [
            ['mail.' . $domainH . ' (A)',            $ipH],
            [$domainH . ' (MX)',                     'mail.' . $domainH],
            [$domainH . ' (TXT)',                    'v=spf1 a mx -all'],
            ['_dmarc.' . $domainH . ' (TXT)',        'v=DMARC1;p=none'],
        ];
        foreach ($dnsRows as $row) {
            $html .= '<div class="row">';
            $html .= '<div class="col-sm-5 text-right"><strong>' . $row[0] . ':</strong></div>';
            $html .= '<div class="col-sm-7"><pre style="margin:0">' . $row[1] . '</pre></div>';
            $html .= '</div>';
        }
